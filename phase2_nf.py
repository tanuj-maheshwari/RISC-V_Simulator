...
def decode():
    global MuxRs1, RA, RB, reg_file, MuxA, MuxB, ISB_FD, ISB_DE, control, end_inst, stall, flush, btb, hazard_unit, data_inst, control_inst, alu_inst, tot_stalls, tot_hit, tot_miss, tot_data_hazards, tot_control_hazards, data_stall, control_stall
    ...
    if end_inst is False:
        stall = hazard_unit.getStalls(instruction)
    else:
        stall = 0
...
def execute():
    global alu, RZ, ISB_DE, ISB_EM, MuxA, MuxB
    ...
    return True, control_signals, RZ.read(), ISB_vals['rs2_val'], ISB_vals['rd'], ISB_vals['PC_val'], instruction_code


def memory_access():
    global memory, MuxY, RY, ISB_EM, ISB_MW, end_exec
...
while True:
    ...
    if e_run:
        ISB_EM.update_EM(EM_tuple[0], EM_tuple[1], EM_tuple[2], Register(
        ).read(), EM_tuple[3], EM_tuple[4], EM_tuple[5], EM_tuple[6])
    else:
...
print("Branch Misredictions :-", tot_miss)
